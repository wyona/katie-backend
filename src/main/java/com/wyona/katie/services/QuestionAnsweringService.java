...
public class QuestionAnsweringService {
    ...
    private List<Hit> generateAnswer(Sentence question, Context domain, List<Hit> hits) {
        ...
        try {
            ...
            if (hits.size() > 0 || alwaysUseLLM) {
                ...
                String completedText = generateProvider.getCompletion(promptMessages, null, model, temperature, apiToken);
                ...
                if (topRetrievalResult != null) {
                    ...
                    // TODO: Add more relevant contexts than just top retrieval result
                    URI uri = null;
                    if (url != null) {
                        uri = new URI(url);
                        hits.get(0).getAnswer().setUrl(url); // TODO: Why do we overwrite this URL, isn't it set already?!
                    }
                    hits.get(0).getAnswer().addRelevantContext(new AnswerContext(context, uri));
                } else {
                    log.warn("No results retrieved, answer was generated by LLM without augmentation.");
    ...
    public ResponseAnswer mapAnswer(Answer answer, Date dateSubmitted, PermissionStatus permissionStatus) {
        ...
        ResponseAnswer responseAnswer = new ResponseAnswer(answer.getUuid(), answer.getSubmittedQuestion(), dateSubmitted, answer.getAnswer(), answer.getAnswerContentType(), answer.getClassifications(), new Date(answer.getDateAnswerModified()), cipherAlgorithm, null, answer.getOriginalquestion(), new Date(answer.getDateOriginalQuestion()), permissionStatus, answer.getType(), answer.getUrl());
        for (AnswerContext context : answer.getRelevantContexts()) {
            responseAnswer.addRelevantContext(context);
        }
